...
cur.execute('CREATE TABLE IF NOT EXISTS UserInfo (id TEXT, pw TEXT ,nickname TEXT)')    #generate database with id, pw and nickname as column when cupet db does not exist
con.commit()
...
def join():
    ...
    if input_PW.get() == input_chk_PW.get():
        ...
        cur.execute("INSERT INTO UserInfo VALUES(?, ?, ?)", (input_ID.get(), input_PW.get(), "None"))   #register id and pw values in cupet.db
